...
export type ErrorTypes = DataErrorAuthTypes | DataErrorCheckoutTypes | ErrorCartTypes;

export class ErrorListener implements IErrorListener {
    private errorListeners: Array<(error: ApolloErrorWithUserInput | any, type: ErrorTypes) => any>;

    constructor() {
        this.errorListeners = [];
    }

    addOnErrorListener = (func: (error: ApolloErrorWithUserInput | any, type: ErrorTypes) => any) => {
    ...
    protected fireError = (error: ApolloErrorWithUserInput | any, type: ErrorTypes) => {
