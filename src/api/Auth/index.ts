...
export const BROWSER_NO_CREDENTIAL_API_MESSAGE =
    "Mzawadie SDK is unable to use browser Credential Management API.";
...
export class AuthAPI extends ErrorListener {
    ...
    registerAccount = async (
        ...
        const { data, dataError } = await this.jobsManager.run("auth", "registerAccount", {
            email,
            password,
            redirectUrl,
        });
    ...
    resetPasswordRequest = async (
        ...
        const { data, dataError } = await this.jobsManager.run("auth", "resetPasswordRequest", {
            email,
            redirectUrl,
        });
    ...
    signIn = async (
        ...
        const { data: userData, dataError: userDataError } = await this.jobsManager.run(
            "auth",
            "provideUser",
            undefined
        );
     ...
     */
    refreshSignInToken = async (refreshToken?: string): PromiseRunResponse<DataErrorAuthTypes> => {
        const { data, dataError } = await this.jobsManager.run("auth", "refreshSignInToken", {
            refreshToken,
        });
    ...
    private autoSignIn = async () => {
        ...
        if (credentials && "password" in credentials && credentials.password) {
            const { dataError } = await this.signIn(credentials.id, credentials.password, true);
