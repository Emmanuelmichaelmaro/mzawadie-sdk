...
export class JobsManager {
    ...
    constructor(localStorageHandler: LocalStorageHandler, apolloClientManager: ApolloClientManager) {
        ...
        this.queuedJobs = new QueuedJobs(this.localStorageHandler, apolloClientManager);
     ...
     * @param jobName Jobs within group/class.
     */
    addToQueue<G extends keyof IQueuedJobs, J extends keyof IQueuedJobs[G]>(jobGroup: G, jobName: J) {
        if (navigator.onLine) {
     ...
     */
    attachEventListener<G extends keyof IJobs, P extends IJobs[G]["attachEventListener"]>(
        jobGroup: G,
     ...
     */
    attachErrorListener<G extends keyof IQueuedJobs, P extends IQueuedJobs[G]["attachErrorListener"]>(
        jobGroup: G,
    ...
    private enqueueJob<G extends keyof IQueuedJobs, J extends keyof IQueuedJobs[G]>(
        jobGroup: G,
        jobName: J
    ) {
    ...
    private dequeueJob<G extends keyof IQueuedJobs, J extends keyof IQueuedJobs[G]>(
        jobGroup: G,
        jobName: J
    ) {
        const methodName = jobName.toString();
    ...
    private updateJobStateInRepository<G extends keyof IQueuedJobs, J extends keyof IQueuedJobs[G]>(
        jobGroup: G,
        jobName: J,
        state: boolean
    ) {
