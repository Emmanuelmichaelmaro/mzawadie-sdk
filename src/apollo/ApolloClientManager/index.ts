...
import { CompleteCheckout, CompleteCheckoutVariables } from "../mutations/gqlTypes/CompleteCheckout";
import { CreateCheckout, CreateCheckoutVariables } from "../mutations/gqlTypes/CreateCheckout";
import {
    CreateCheckoutPayment,
    CreateCheckoutPaymentVariables,
} from "../mutations/gqlTypes/CreateCheckoutPayment";
import { RefreshToken, RefreshTokenVariables } from "../mutations/gqlTypes/RefreshToken";
import { RegisterAccount, RegisterAccountVariables } from "../mutations/gqlTypes/RegisterAccount";
import {
...
import { TokenAuth, TokenAuthVariables } from "../mutations/gqlTypes/TokenAuth";
import {
...
import { VerifyToken, VerifyTokenVariables } from "../mutations/gqlTypes/VerifyToken";
...
export class ApolloClientManager {
    ...
    subscribeToUserChange = (
        ...
        this.client
            ...
            .subscribe((value) => next(value.data?.user), error, complete);
    };
    ...
    registerAccount = async (email: string, password: string, redirectUrl: string) => {
        const { data, errors } = await this.client.mutate<RegisterAccount, RegisterAccountVariables>({
            fetchPolicy: "no-cache",
            mutation: UserMutations.registerAccountMutation,
            variables: {
                email,
                password,
                redirectUrl,
    ...
    resetPasswordRequest = async (email: string, redirectUrl: string) => {
        ...
        >({
            fetchPolicy: "no-cache",
            mutation: UserMutations.resetPasswordRequestMutation,
            variables: {
                email,
                redirectUrl,
            },
    ...
    refreshSignInToken = async ({ csrfToken, refreshToken }: RefreshSignInTokenInput) => {
        const { data, errors } = await this.client.mutate<RefreshToken, RefreshTokenVariables>({
            fetchPolicy: "no-cache",
    ...
    getCheckout = async (isUserSignedIn: boolean, channel: string, checkoutToken: string | null) => {
    ...
    getRefreshedCheckoutLines = async (checkoutlines: ICheckoutModelLine[] | null, channel: string) => {
            ...
            const variables = {
                checkoutInput: {
                    ...
                    shippingAddress: shippingAddress && {
                        ...
                        country:
                            CountryCode[shippingAddress?.country?.code as keyof typeof CountryCode],
    ...
    setBillingAddress = async (billingAddress: ICheckoutAddress, checkoutId: string) => {
        try {
            const variables = {
                billingAddress: {
                    city: billingAddress.city,
    ...
    addPromoCode = async (promoCode: string, checkoutId: string) => {
        try {
            const { data, errors } = await this.client.mutate<
                ...
                AddCheckoutPromoCodeVariables
            >({
                mutation: CheckoutMutations.addCheckoutPromoCodeMutation,
    ...
    removePromoCode = async (promoCode: string, checkoutId: string) => {
        try {
            const { data, errors } = await this.client.mutate<
                ...
                RemoveCheckoutPromoCodeVariables
            >({
                mutation: CheckoutMutations.removeCheckoutPromoCodeMutation,
                variables: { checkoutId, promoCode },
            });
    ...
    createPayment = async ({ amount, checkoutId, gateway, token, returnUrl }: CreatePaymentInput) => {
        try {
            const variables = {
                checkoutId,
                paymentInput: {
