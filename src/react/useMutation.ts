...
>(
    ...
    const [result, setResult] = React.useState<MutationResult<TData>>(initialState);
    ...
    const handleMutationError = (error: ApolloErrorWithUserInput, mutationId: number) => {
    ...
    const runMutation = React.useCallback(
        (variables: TVariables, options: TOptions) => {
            return new Promise((resolve) => {
                ...
                (mzawadie.legacyAPIProxy[mutation] as (variables: any, options: any) => Promise<any>)(
                    apolloVariables,
                    apolloOptions
                )
                    .then((data) => {
...
export const mutationFactory =
    <T extends keyof APIProxy, TVariables extends Variables<T>, TOptions extends Options<T>>(
        mutation: T
    ) =>
    (variables?: TVariables, options?: TOptions) =>
