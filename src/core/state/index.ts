...
import { PaymentGateway } from "../../apollo/types";
...
import { IMzawadieStateSummeryPrices, StateItems } from "./types";

export interface MzawadieStateLoaded {
    user: boolean;
    signInToken: boolean;
    checkout: boolean;
    payment: boolean;
...
const defaultMzawadieStateLoaded = {
    checkout: false,
    payment: false,
    signInToken: false,
    summaryPrices: false,
...
export class MzawadieState extends NamedObservable<StateItems> {
    user?: User | null;
    ...
    constructor(
        ...
        this.loaded = defaultMzawadieStateLoaded;
    ...
    /**
     * Subscribes to particular changes occurring in data sources like apollo cache or local storage.
     * Every update in data source will result in update of respective class member.
     */
    private subscribeStateToChanges = () => {
        this.localStorageHandler.subscribeToChange(LocalStorageItems.CHECKOUT, this.onCheckoutUpdate);
        this.localStorageHandler.subscribeToChange(LocalStorageItems.PAYMENT, this.onPaymentUpdate);
        this.localStorageHandler.subscribeToChange(LocalStorageItems.TOKEN, this.onSignInTokenUpdate);
        this.localStorageHandler.subscribeToChange(LocalStorageEvents.CLEAR, this.onClearLocalStorage);
    ...
    private verityToken = async () => {
        const { data, dataError } = await this.jobsManager.run("auth", "verifySignInToken", undefined);
    ...
    private onLoadedUpdate = (newLoaded: Partial<MzawadieStateLoaded>) => {
        this.loaded = {
            ...this.loaded,
            ...newLoaded,
        };
    ...
    private onSignInTokenVerifyingUpdate = (tokenVerifying: boolean) => {
        ...
        this.notifyChange(StateItems.SIGN_IN_TOKEN_VERIFYING, this.signInTokenVerifying);
    };

    private onSignInTokenRefreshUpdate = (tokenRefreshing: boolean) => {
        this.signInTokenRefreshing = tokenRefreshing;
        this.notifyChange(StateItems.SIGN_IN_TOKEN_REFRESHING, this.signInTokenRefreshing);
    };
    ...
    private onCheckoutUpdate = (checkout?: ICheckoutModel) => {
        ...
        this.summaryPrices = MzawadieState.calculateSummaryPrices(checkout);
    ...
    private static calculateSummaryPrices(checkout?: ICheckoutModel): IMzawadieStateSummeryPrices {
        ...
        if (items && items.length) {
            ...
            if (firstItemTotalPrice) {
                ...
                const totalPrice = {
                    ...
                    gross: {
                        ...subtotalPrice.gross,
                        amount: round(itemsGrossPrice + shippingPrice.amount - discount.amount, 2),
                    },
                    net: {
                        ...subtotalPrice.net,
                        amount: round(itemsNetPrice + shippingPrice.amount - discount.amount, 2),
                    },
                };
